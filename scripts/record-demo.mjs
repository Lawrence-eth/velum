...
try{
 await page.goto('https://velum.aethe.me',{waitUntil:'networkidle'});await scene('One task: pay an invoice without accepting its instructions','.flow-heading',15);
 await scene('Suspicious remittance instruction and the verified wallet','.flow-grid',15);await page.locator('#run-agent').click();await completed();
 assert.match(await page.locator('#result-source').innerText(),/LIVE MODEL/);
 await scene('Actual live model proposal and actual local treasury result','#payment-result',23);
 ...
 await scene('The Chainlink confidential-handler connection','#evidence',24);
 ...
 await page.goto('https://velum.aethe.me',{waitUntil:'networkidle'});await scene('Close: one guided payment run','.flow-heading',15);
}finally{await context.close();await video.saveAs('artifacts/velum-guided-demo-footage.webm');await browser.close();writeFileSync('artifacts/guided-demo-cues.json',JSON.stringify({note:'Silent footage requiring human narration. Actual live inference and local EVM; CRE evidence recorded separately.',cues},null,2)+'\n');}
