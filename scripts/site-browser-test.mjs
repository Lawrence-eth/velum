...
try{
 for(const [path,label] of routes){
  ...
  if(path==='/evidence.html'){await page.locator('#work-records tbody tr').first().waitFor();assert.equal(await page.locator('#work-records tbody tr').count(),2);await page.locator('#agent-records tbody tr').first().waitFor();assert.equal(await page.locator('#agent-records tbody tr').count(),3);await page.locator('#sepolia-records .proof-cards').waitFor();assert.equal(await page.locator('#sepolia-records a[href*="/tx/"]').count(),3)}
