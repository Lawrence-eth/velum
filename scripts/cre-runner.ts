import {pendingWorkResults,acknowledgeWork} from './work-journal';
...
while(true){
 try{for(const pending of pendingWorkResults()){await api({action:'runner-result',id:pending.id,execution:pending.execution});acknowledgeWork(pending.workspace,pending.id);}
 const {job}=await api({action:'runner-claim'});if(job){console.log(`Executing ${job.id}`);const heartbeat=setInterval(()=>{void api({action:'runner-claim'}).catch(()=>{});},10000);try{const execution=await executeJob(job);await api({action:'runner-result',id:job.id,execution});if(job.capture.work)acknowledgeWork(job.capture.work.workspace,job.id);console.log(`Completed ${job.id}: approved=${execution.policyApproved}`);}catch(error){console.error(`Failed ${job.id}: ${error instanceof Error?error.message:'Execution failed'}`);await api({action:'runner-result',id:job.id,failed:true});}finally{clearInterval(heartbeat);}}}
