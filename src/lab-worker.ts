...
import {agentBatch,proposalSchema,type Proposal} from './agent';
...
async function run(input:{attack:string;proposal?:Proposal;runId?:string;contracts:Record<string,{abi:any;bytecode:Hex}>}){
 const allowed=['valid','recipient','amount','reorder','omit','identity','caller','replay','transfer','proposal'];
 ...
 const batch=input.attack==='proposal'
  ? agentBatch(proposalSchema.parse(input.proposal),now,typeof input.runId==='string'&&input.runId.length<=100?input.runId:crypto.randomUUID())
  : batchFixture(now,6000,treasury.address.toString(),token.address.toString() as Hex);
 if(input.attack==='proposal')for(const entry of batch.entries){entry.bundle.request.consumer=treasury.address.toString() as Hex;entry.bundle.request.token=token.address.toString() as Hex;entry.bundle.policy.token=token.address.toString() as Hex;}
 const evaluation=evaluateBatch(batch,now);
 ...
 if(!(await call(token,'mint',[treasury.address.toString(),20000000000n])).success)throw new Error('Funding failed');if(!(await call(treasury,'openBatch',[batch.batchId,payments])).success)throw new Error('Payment registration failed');
