...
export type AgentCapture={ok:true;runId:string;createdAt:number;source:'live-model'|'injected-proposal'|'operator-corrected';scenario:'clean'|'malicious';synthetic:true;creExecution:false;settlement:false;proposal:Proposal;gate:Verdict;commitment:string;bundle:Bundle;raw?:string;model?:string;creAccess?:string};
export type JobResponse={ok:boolean;status?:string;error?:string;execution?:JobResult};
export const jobResultSchema=z.object({
 creExecution:z.literal(true),proposal:proposalSchema,policyApproved:z.boolean(),policyReasons:z.array(z.string().max(150)),deliveryAccepted:z.literal(true),settlementAccepted:z.boolean(),
 ...
 treasury:z.string(),token:z.string(),workflowMode:z.literal('CRE CLI confidential-handler simulation'),settlementMode:z.literal('VM-local Solidity execution with mock forwarder'),creLog:z.string().max(60000),executedAt:z.string(),
}).strict();
